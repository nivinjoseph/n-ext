...
suite("StringExt", function () {
    suite("isEmptyOrWhiteSpace", function () {
        test("should return true when called on an empty string", function () {
            var target = "";
            var result = target.isEmptyOrWhiteSpace();
    ...
    suite("contains", function () {
        ...
        test("should return false if argument is partially of the target string", function () {
            var target = "foo bar";
            var arg = "foo baz";
            var result = target.contains(arg);
            assert.strictEqual(result, false);
    ...
    suite("startsWith", function () {
        ...
        test("should return false if the target string is an empty string and the argument has a single character", function () {
            var target = "";
            var arg = "a";
            var result = target.startsWith(arg);
            assert.strictEqual(result, false);
        ...
        test("should return true if target string is a space character and the argument is a non space character", function () {
            var target = " ";
            var arg = "";
            var result = target.startsWith(arg);
            assert.strictEqual(result, true);
        });
        test("should return false if target is an empty string and the argument is a space character", function () {
            var target = "";
            var arg = " ";
            var result = target.startsWith(arg);
            assert.strictEqual(result, false);
    ...
    suite("format", function () {
        test("should return formated string with arguments in order", function () {
            var target = "my name is {0} {1}";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "my name is Viola Deluca");
        });
        test("should return formated string containing only first argument twice", function () {
            var target = "my name is {0} {0}";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "my name is Viola Viola");
        });
        test("should return formated string with arguments in reverse order", function () {
            var target = "my name is {1} {0}";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "my name is Deluca Viola");
        });
        test("should return formated string containing only first argument, leaving {2} unchanged", function () {
            var target = "my name is {0} {2}";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "my name is Viola {2}");
        });
        test("should return an empty string if target is an empty string and argument has value", function () {
            var target = "";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "");
        });
        test("should return original target unformated when target provides no placeholders", function () {
            var target = "my name is";
            var result = target.format("Viola", "Deluca");
            assert.strictEqual(result, "my name is");
        });
        test("should return formated string with placeholder replaced by empty string if target has placeholders and argument provided is an empty string", function () {
            var target = "my name is {0} {1}";
            var result = target.format("");
            assert.strictEqual(result, "my name is  {1}");
        });
        test("should return original string if target has placeholders but no arguments are provided", function () {
            var target = "my name is {0} {1}";
            var result = target.format();
            assert.strictEqual(result, "my name is {0} {1}");
        });
        test("should return formated string with first two arguments when there are more arguments than placeholders", function () {
            var target = "my name is {0} {1}";
            var result = target.format("Viola", "Deluca", "Nivin", "Joseph");
            assert.strictEqual(result, "my name is Viola Deluca");
        });
        test("should return formated string with only first placeholder changed given a single argument", function () {
            var target = "my name is {0} {1}";
